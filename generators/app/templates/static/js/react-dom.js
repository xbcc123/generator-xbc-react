	...
	var eventResponderContext = {
			...
			switch (eventPriority) {
				case DiscreteEvent: {
					...
					discreteUpdates(function() {
						return executeUserEventHandler(
							eventListener,
							eventValue
						);
					});
		...
		isTargetWithinResponder: function(target) {
			...
			if (target != null) {
				...
				while (fiber !== null) {
					if (
						fiber === responderFiber ||
						fiber.alternate === responderFiber
					) {
						return true;
					}
	...
	function replayUnblockedEvents() {
		...
		while (queuedDiscreteEvents.length > 0) {
			...
			if (nextDiscreteEvent.blockedOn !== null) {
				...
				var _fiber4 = getInstanceFromNode$1(
		...
		if (!listeningSet.has(topLevelType)) {
			switch (topLevelType) {
				...
				default:
					...
					var isMediaEvent =
						mediaEventTypes.indexOf(topLevelType) !== -1;
	...
	function warnAboutUpdateOnUnmountedFiberInDEV(fiber) {
		{
			...
			if (didWarnStateUpdateForUnmountedComponent !== null) {
				if (
					didWarnStateUpdateForUnmountedComponent.has(componentName)
				) {
					return;
				}
						...
						switch (workInProgressNode.tag) {
							...
							case SimpleMemoComponent:
								if (
									...
									workInProgressNode.memoizedState
										.baseUpdate !== null
								) {
									var _update =
										workInProgressNode.memoizedState
											.baseUpdate; // Loop through the functional component's memoized state to see whether
